...
class OpenAIRealtimeLMM(OpenAIAPI):
    ...
    async def lmm_session_prepare(  # noqa: C901, PLR0915
        ...
        **extra: Any,
        ...
        # prepare config
        tools = tools if tools is not None else LMMTools.none
        memory = memory if memory is not None else LMMMemory.none
        ...
        async def open_session() -> LMMSession:  # noqa: C901, PLR0915
            ...
            # initialize context
            context: LMMContext = await memory.recalling()
            if context:
                await _send_context(
                    context,
                    ...
                    match event.type:
                        ...
                        case "response.done":
                            ...
                            if event.response.status == "incomplete":
                                continue  # TODO: handle incomplete
