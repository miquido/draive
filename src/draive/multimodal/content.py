...
class MultimodalContent(DataModel):
    ...
    def extract[Result](
        ...
        /,
        *,
        conversion: Callable[[Self], Result],
    ...
    def extract[Result](
        ...
        /,
        *,
        conversion: Callable[[Self], Result] | None = None,
    ) -> Generator[Self, None] | Generator[Result, None]:
        return _extract_tags(  # pyright: ignore[reportReturnType]
            tag,
            source=self,
    ...
    def extract_first[Result](
        ...
        *,
        ...
        return next(
            self.extract(  # pyright: ignore[reportUnknownVariableType, reportUnknownArgumentType]
                tag,
                conversion=conversion,  # pyright: ignore[reportArgumentType]
            ),
            None,
...
def _extract_tags[Result](  # noqa: C901, PLR0912, PLR0915
    tag: str,
    /,
    ...
    tag_opening_extended_prefix: str = f"<{tag} "
                    ...
                    if tag_accumulator:  # check the inside of potential tag
                        ...
                        if char == ">":
                            ...
                            if tag_accumulator == tag_closing_prefix:
                                tag_content: MultimodalContent = MultimodalContent.of(
                                    *(tag_content_parts or [])
                                )

                                if conversion:
                                    yield conversion(tag_content)
                            ...
                            # we have found opening tag
                                ...
                                tag_accumulator == tag_opening_prefix
                                or tag_accumulator.startswith(tag_opening_extended_prefix)
                            ):
                                # we have completed tag detection
                                tag_accumulator = ""
                                ...
                                if tag_parts:
                                    if tag_content_parts is not None:
