...
from haiway import AttributePath, AttributeRequirement, State, ctx, retry
...
@final
class SurrealVectorIndex:
    """SurrealDB-backed vector index factory.

    Exposes static helpers for schema migration and for creating a
    :class:`~draive.embedding.VectorIndex` implementation persisting entries in
    SurrealDB. Each indexed model uses its own table, which has to be defined
    with :meth:`migrate` before indexing or searching it.

    Examples
    --------
    ```python
    from draive import ctx
    from draive.surreal import SurrealClient, SurrealVectorIndex

    async def bootstrap_index() -> None:
        async with SurrealClient(url="ws://localhost:8000/rpc") as surreal:
            async with ctx.scope("vector-index", surreal):
                await SurrealVectorIndex.migrate(Document, dimensions=1536)

            async with ctx.scope("search", surreal, SurrealVectorIndex.prepare()):
                ...
    ```
    """

    @staticmethod
    async def migrate[Model: State](
        model: type[Model],
        /,
        *,
        dimensions: int,
        ...
        """Define the database structures required to index a model.

        Defines the model's table and its HNSW embedding index when not already
        defined. A SurrealDB server refuses to read from a table which was never
        defined nor written to, and the KNN operator requires the index,
        therefore both have to be defined upfront.

        Parameters
        ----------
        model
            Model type to define the structures for, its name is used as the
            table name.
        dimensions
            Dimension of the embedding vectors stored within the table, it has
            to match the embedding used when indexing.
        vector_type
            SurrealDB vector storage type for the HNSW index.
        distance
            SurrealDB distance metric used by the HNSW index, it has to match
            the one passed to :meth:`prepare`.
        efc
        ...
        ------
        ValueError
            Raised when any of the provided values is invalid.
        SurrealException
            Raised when SurrealDB statement execution fails.
        """
        if dimensions <= 0:
            raise ValueError("dimensions has to be greater than 0")
        ...
        table: str = surreal_identifier(model.__name__)
        options: list[str] = [
            f"TYPE {_surreal_option_token(vector_type, name='vector_type')}",
            f"DIST {_surreal_option_token(distance, name='distance')}",
        ]
        if efc is not None:
        ...
        await Surreal.execute(f"DEFINE TABLE IF NOT EXISTS {table} SCHEMALESS TYPE NORMAL;")
        await _define_index(
    ...
    def prepare(  # noqa: C901, PLR0915
        *,
        ...
        Each indexed model requires its table and embedding index to be defined
        with :meth:`migrate` beforehand.
        ...
        distance
            SurrealDB distance metric the embedding indexes were defined with,
            it determines whether ``score_threshold`` is supported.
        ...
        async def index[Model: State, Value: ResourceContent | TextContent | str](
            ...
            **extra: Any,
            ...
            for idx, (value, embedded) in enumerate(
                zip(indexed_values, embedded_values, strict=True)
            ):
                await Surreal.execute(
                    f"""
                    CREATE {surreal_identifier(model.__name__)} SET
                        content = $content,
                        embedding = $embedding,
                        created = $created;
                    """,
            ...
            if query is None:
                rows: Sequence[SurrealObject] = await Surreal.execute(
                    ...
                    SELECT
                        ...
                        id
                    FROM
                        {surreal_identifier(model.__name__)}
                    {where_clause}
                    ORDER BY
                        created DESC,
                        id DESC
                ...
                return tuple(
                    model.from_mapping(cast(Mapping[str, Any], record["content"]))
                    for record in rows
                )
    ...
    def __init__(self) -> NoReturn:
        raise RuntimeError("SurrealVectorIndex instantiation is forbidden")
...
def _transaction_conflict(
    ...
    return (
        isinstance(exception, SurrealException) and "transaction conflict" in str(exception).lower()
    )


@retry(
    limit=3,
    delay=lambda attempt, _: 0.1 * attempt,
    catching=_transaction_conflict,
)
async def _define_index(
    statement: str,
    /,
) -> None:
    # Defining an index is idempotent but not free of contention - concurrent
    # definitions of a not yet existing index abort with a key-value store
    # conflict, therefore conflicts are retried.
    await Surreal.execute(statement)
