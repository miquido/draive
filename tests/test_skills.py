...
@pytest.mark.asyncio
async def test_skill_from_directory_raises_when_skill_manifest_is_missing(
    tmp_path: Path,
) -> None:
    skill_root = tmp_path / "no-manifest"
...
@pytest.mark.asyncio
async def test_skill_from_directory_rejects_invalid_name_format(
    tmp_path: Path,
) -> None:
    skill_root = tmp_path / "csv-analysis"
    skill_root.mkdir(parents=True)
