from draive import Model, ParameterPath


class SequenceDataModel(Model):
    value: int


class DictDataModel(Model):
    key: str


class NestedDataModel(Model):
    value: float


class RecursiveDataModel(Model):
...
def test_id_path_points_to_self():
    path: ParameterPath[DataModel, DataModel] = DataModel.path_cast(DataModel._)
...
def test_attribute_path_points_to_attribute():
    path: ParameterPath[DataModel, str] = DataModel.path_cast(DataModel._.answer)
...
def test_nested_attribute_path_points_to_nested_attribute():
    path: ParameterPath[DataModel, float] = DataModel.path_cast(DataModel._.nested.value)
...
def test_recursive_attribute_path_points_to_attribute():
    path: ParameterPath[DataModel, RecursiveDataModel] = DataModel.path_cast(DataModel._.recursive)
...
def test_list_item_path_points_to_item():
    path: ParameterPath[DataModel, SequenceDataModel] = DataModel.path_cast(
        DataModel._.list_models[1]
    )
...
def test_mixed_tuple_item_path_points_to_item():
    path: ParameterPath[DataModel, DictDataModel] = DataModel.path_cast(
