import pytest
from draive.splitters.exhausted import split_text
...
@pytest.fixture
def exhausted_splitting_expected_result() -> str:
    with open("tests/data/exhausted_splitting.txt") as file:
        return file.read()


@pytest.fixture
def exhausted_overlap_splitting_expected_result() -> str:
    with open("tests/data/exhausted_overlap_splitting copy.txt") as file:
        return file.read()
...
def test_text_splitting_with_long_text(
    sample_text: str,
    exhausted_splitting_expected_result: str,
) -> None:
    ...
    assert "\n".join(result) == exhausted_splitting_expected_result


def test_uses_overlap_splitting_with_long_text(
    sample_text: str,
    exhausted_overlap_splitting_expected_result: str,
) -> None:
    result: list[str] = split_text(
        text=sample_text,
        part_size=500,
    ...
    assert "\n".join(result) == exhausted_overlap_splitting_expected_result
